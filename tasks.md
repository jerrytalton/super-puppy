...
**Next Step:** Merge via PR and cut a release with `bin/release.sh`. **Known gap before tagging (red-team 2026-08-14):** presets converge fleet-wide within ~2 min of the tag, but *weights don't* — auto-update never pulls models, so existing 64gb/128gb machines will have their warm "general" target point at the un-pulled `qwen3.8:27b-mlx`: keep-warm reload fails silently at debug level every 4-min tick and the model actually serving traffic (qwen3.6:27b-mlx, still resolvable as a fallback pick) loses its warm pings — every text request cold-loads until someone pulls manually. Decide before release: auto-pull the active tier's missing tags on profile migration (pull worker already exists in profile-server), or ship release notes + a menu bar "warm model not installed" surface. Also noted (LOW, not blocking): `lib/models.py` still trusts a `capabilities` key *inside* model_info (inert, pre-existing) three lines under the NOTE disclaiming it.
