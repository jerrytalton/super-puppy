...
# Translates Anthropic /v1/messages format to Ollama/MLX backends so
# Claude Code can use local models as its primary model.

def _anthropic_to_openai_messages(body: dict) -> list[dict]:
    """Convert Anthropic message format to OpenAI/Ollama format."""
    messages = []
    ...
    for msg in body.get("messages", []):
        ...
        # Content is array of blocks — flatten to text
        parts = []
...
def _anthropic_response(content: str, model: str, msg_id: str = "") -> dict:
...
async def _proxy_messages(request):
    """Anthropic /v1/messages proxy — routes to Ollama or MLX."""
    from starlette.responses import StreamingResponse, JSONResponse as StarletteJSON

    try:
        body = await request.json()
    except Exception:
        return StarletteJSON({"error": "Invalid JSON"}, status_code=400)

    requested_model = body.get("model", "")
    max_tokens = body.get("max_tokens", 4096)
    stream = body.get("stream", False)

    # Resolve model: try the requested name first, fall back to SP preferences
    try:
        model, backend = pick_model("general", override=requested_model or None)
    except ValueError as e:
        return StarletteJSON(
            _anthropic_response(str(e), requested_model or "unknown"),
            status_code=200)
    ...
    if stream:
        return StreamingResponse(
            _stream_anthropic(model, backend, openai_messages, max_tokens),
            media_type="text/event-stream",
            headers={"Cache-Control": "no-cache"})

    # Non-streaming (thinking disabled — local model reasoning isn't useful here)
    try:
        if backend == "ollama":
            content = await chat_ollama(model, openai_messages, max_tokens, think=False)
        else:
            content = await chat_mlx(model, openai_messages, max_tokens, think=False)
    except Exception as e:
        content = f"Error: {e}"

    return StarletteJSON(_anthropic_response(content, model))


async def _stream_anthropic(model, backend, messages, max_tokens):
    """Stream Anthropic SSE events from an Ollama/MLX backend."""
    import uuid
    msg_id = f"msg_{uuid.uuid4().hex[:24]}"
    ...
    try:
        if backend == "ollama":
            async for token in _stream_ollama(model, messages, max_tokens):
                output_tokens += 1
                yield f"event: content_block_delta\ndata: {json.dumps({'type': 'content_block_delta', 'index': 0, 'delta': {'type': 'text_delta', 'text': token}})}\n\n"
        else:
            async for token in _stream_mlx(model, messages, max_tokens):
                output_tokens += 1
                yield f"event: content_block_delta\ndata: {json.dumps({'type': 'content_block_delta', 'index': 0, 'delta': {'type': 'text_delta', 'text': token}})}\n\n"
    except Exception as e:
...
async def _stream_ollama(model, messages, max_tokens):
    """Yield tokens from Ollama streaming."""
...
async def _stream_mlx(model, messages, max_tokens):
    """Yield tokens from MLX streaming."""
    body = {"model": model, "messages": messages, "max_tokens": max_tokens,
            "stream": True}
    async with httpx.AsyncClient(timeout=300) as client:
        async with client.stream("POST", f"{MLX_URL}/v1/chat/completions", json=body) as resp:
            resp.raise_for_status()
            async for line in resp.aiter_lines():
                if not line:
                    continue
...
async def _proxy_count_tokens(request):
    ...
    try:
        ...
        return StarletteJSON({"input_tokens": 0}, status_code=200)
    # Rough estimate: 4 chars per token
    text = json.dumps(body.get("messages", []))
    return StarletteJSON({"input_tokens": len(text) // 4})
